...
ismerge=0 AND status=3 AND (paytype=1 OR paytype=2 OR paytype=4) ";
        if ($is_contain_delivery == 1) { //包含配送费
            $order_totalprice = pdo_fetchcolumn("SELECT sum(totalprice-one_order_getprice-refund_price) FROM " . tablename($this->table_order) . " WHERE {$strwhere}", array(':weid' => $weid, ':storeid' => $storeid));
            $order_totalprice = floatval($order_totalprice);
        } else {
            //不包含配送费
            $time = strtotime('2017-07-17 00:00:00');
            $order_totalprice1 = pdo_fetchcolumn("SELECT sum(totalprice-one_order_getprice-refund_price) FROM " . tablename($this->table_order) . " WHERE {$strwhere} AND
dateline<:dateline", array(':weid' => $weid,
                ':storeid' => $storeid, ':dateline' => $time));
            $order_totalprice1 = floatval($order_totalprice1);

            $order_totalprice2 = pdo_fetchcolumn("SELECT sum(totalprice-dispatchprice-one_order_getprice-refund_price) FROM " . tablename($this->table_order) . " WHERE {$strwhere}
...
//                $content .= '备注:' . $order['remark'] . $huanhang;
            ...
            if ($value['is_print_all'] == 1) {
                ...
                foreach ($ordergoods as $v) {
                    if ($goods[$v['goodsid']]) {
                        ...
                        $content1 .= '<B>'.$goodsname . ' ' . $v['total'] . $goods[$v['goodsid']]['unitname'] . ' ' . number_format($money, 2) . "元\n</B>";
                    }
                }
    ...
    // 飞鹅退菜或换桌
    public function _feietui($tableid,$caiid,$orderid,$content=""){
        $table = pdo_fetch("SELECT * FROM " . tablename($this->table_tables) . " WHERE id = :tableid limit 1", array(':tableid' => $tableid));
        $cai = pdo_fetch("SELECT * FROM " . tablename($this->table_goods) . " WHERE id = :caiid limit 1", array(':caiid' => $caiid));
        $order = pdo_fetch("SELECT * FROM " . tablename($this->table_order) . " WHERE id = :orderid limit 1", array(':orderid' => $orderid));
        if(empty($table) || empty($cai) || empty($order)) return ;

        $print_content = "桌号:{$table['title']}\n";
        $print_content .= "<BR>-------------------------<BR>";
        $print_content .= "退菜:{$cai['title']}\n";

        $content = $content?$content:$print_content;
        //前台 后厨凉菜 后厨热菜
        $settings = pdo_fetchall("SELECT * FROM " . tablename($this->table_print_setting) . " WHERE id in (2,3,4)");
        foreach ($settings as $item => $value) {
            $this->member_code = $value['member_code'];
            $this->device_no = $value['print_usr'];
            $this->feyin_key = $value['feyin_key'];
    ...
    //飞鹅
    public function _feieSend($orderids, $position_type = 0, $isend = 0,$dyue=0,$vipyue=0,$dcard='',$vipcard='')
    {
        global $_W, $_GPC;
        $weid = $this->_weid;
        $totalprice = 0;
        ...
        foreach ($orderids as $k => $id) {
            ...
            $store = $this->getStoreById($storeid);
            $orderinfo_content .= "<B>-------------------------</B><BR>";
            $orderinfo_content .= '<B>订单编号:' . $order['ordersn'] . "</B><BR>";
            
            $service_money = floatval($order['service_money']);
            if ($service_money > 0) {
                $service_money_content = "<B>服务费:{$service_money}元</B><BR>";
            }

            $tables_content = '<B>桌台信息:' . $this->getTableName($order['tables']) . "</B><BR>";
            $totalprice = $totalprice + floatval($order['totalprice']);
            $totalprice_content = "<B>总价:" . number_format($totalprice, 2) . "元</B><BR>";
            ...
            foreach ($goods as $v) {
                ...
                $orderinfo_content .= '<B>'.$goodsname . ' ' . $goodsid[$v['id']]['total'] . $v['unitname'] . ' ' .
                    number_format($money, 2) . "元" . ' ' . $goodstotalprice . "元</B><BR>";
            }
        }
        if ($isend == 1) {
            $print_content = "<B>商家小票(已支付)</B>\n";
        } else {
            $print_content = "<B>商家小票</B>\n";
        }
        $orderidstr = implode(',', $orderids);
        $tableorder = pdo_fetch("SELECT paytype,totalprice,money FROM " . tablename($this->table_tableorder) . " WHERE orderids=:orderids limit 1", array(':orderids'=>$orderidstr));
        if(!empty($tableorder)){
            if ($isend == 1) {
                $str = $paytype[$tableorder['paytype']];
                if(!empty($dyue)) $str.= '<BR>股东卡'.$dcard.'余额:'.$dyue;
                if(!empty($vipyue)) $str.= '<BR>vip卡'.$vipcard.'余额:'.$vipyue;
                $orderinfo_content .= "<B>-------------------------</B><BR>";
                $orderinfo_content .= '<B>支付方式:' . $str . "</B><BR>";
            }
            $totalprice_content .= "<B>应收:" . number_format($tableorder['totalprice'], 2) . "元</B><BR>";
            $totalprice_content .= "<B>实收:" . number_format($tableorder['money'], 2) . "元</B><BR>";
        }

        
        $print_content .= "<B>-------------------------</B><BR>";
        $print_content .= $tables_content;
        $print_content .= $orderinfo_content;
        $print_content .= "<B>-------------------------</B><BR>";
        $print_content .= $totalprice_content;
...
DESC LIMIT 1", array(':weid' => $this->_weid, ':goodsid' => $item['goodsid']));
        ...
        // 退菜
        if(!empty($goodsnum)){
            $this->_feietui($order['tables'],$goods['id'],$orderid); 
        }
...
DESC LIMIT 1", array(':tid' => $orderid, ':uniacid' => $this->_weid));
    ...
    public function doMobilePayTip()
    {
        ...
        $paylog = pdo_fetch("SELECT * FROM ims_weisrc_dish_print_order WHERE orderid=:orderid limit 1", array(':orderid' => $orderid));
        if(empty($paylog)){
            $this->feiyinSendFreeMessage($orderid); //飞印
            $this->_365SendFreeMessage($orderid); //365打印机
            $this->feieSendFreeMessage($orderid); //飞鹅
