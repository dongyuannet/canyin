...
if ($operation == 'fengniaolist') {
    ...
    if (isset($_GPC['status']) && $_GPC['status'] != '') {
        $sta = " AND status = '" . intval($_GPC['status']) . "'";
    }
    if(!empty($_GPC['ordersn']) || !empty($_GPC['tableid'])) $sta = '';
    $commoncondition .= $sta;
    ...
    if (!empty($_GPC['confirmtable'])) {
        ...
        // 换桌
        if(!empty($_GPC['tableid'])){
            $otable = pdo_fetch("SELECT * FROM " . tablename($this->table_tables) . " WHERE id=:id limit 1", array(':id' => $order['tables']));
            $print_content = "换桌【{$otable['title']}】 -- 换 -- 【{$table['title']}】";
            $this->_feietui($order['tables'],1,$order['id'],$print_content); 
        }
