...
weid=:weid
 AND storeid=:storeid AND status=3 AND ispay=1 $condition ", array(':weid' => $this->_weid, ':storeid' => $storeid));
    ...
    $shi_total_order_price = pdo_fetchcolumn("SELECT sum(money) FROM " . tablename($this->table_tableorder) );
...
if ($operation == 'out') {
    ...
    if($time == 'yesmorning'){
        ...
        $end = date('Y-m-d',strtotime("-1 days")).' 22:00:00';
    }
    if($time == 'yes'){
        $start = date('Y-m-d',strtotime("-1 days")).' 22:00:00';
    ...
    header("Content-Disposition:filename={$start}.xls");

    // paytype统计
    $payres = pdo_fetchall("SELECT sum(totalprice) as yf ,sum(money) as sf,paytype FROM " . tablename($this->table_tableorder) . " WHERE created_at >= '{$start}' and created_at <= '{$end}' group by paytype");
    $yf = $sf = 0;

    foreach ($payres as $k => $v) {
        $yf += $v['yf'];
        $sf += $v['sf'];
        $payres[$k]['paytypename'] = $paytype[$v['paytype']];
        echo "{$paytype[$v['paytype']]}\t应付:{$v['yf']}\t实付:{$v['sf']}\r";
    }

    echo "总计\t应付:$yf\t实付:$sf\r";
   
    echo "\r菜单统计\r";
    
    $info = pdo_fetchall("SELECT * FROM " . tablename($this->table_tableorder) . " WHERE created_at >= '{$start}' and created_at <= '{$end}'");
    $orderids = [];
    foreach ($info as $k => $v) {
        $temp = explode(",", $v['orderids']);
        foreach ($temp as $key => $value) {
            $orderids[] = $value;
        }
    }
    $orderidstr = implode(",", $orderids);
    $res = pdo_fetchall("select sum(total) as a,title from (SELECT g.title,og.total,og.orderid FROM " . tablename($this->table_order_goods) . " as og join ".tablename($this->table_goods)." as g on g.id=og.goodsid WHERE og.orderid in ({$orderidstr})) a group by a.title");
    
    foreach ($res as $key => $v) {
        echo "{$v['title']}\t数量:{$v['a']}\r";
    }
    die;
