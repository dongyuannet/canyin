...
<div class="main" id="coupon_display">
...
<?php  if(COUPON_TYPE == WECHAT_COUPON) { ?>
<a class="btn btn-success js-sync pull-left" onclick="util.ajaxshow('<?php  echo $this->createWeburl('couponmanage', array('op' => 'sync', 'type' => 1, 'page' => $_GPC['page']))?>')">更新本页卡券最新状态</a>&nbsp;&nbsp;
<a class="btn btn-success js-download" click="download(0);">同步卡券信息</a>
<?php  } ?>
...
<script type="text/javascript">
	...
	function download(offset) {
		util.message('正在下载卡券数据...');
		$.post(download_url, {offset : offset}, function(data) {
			data = $.parseJSON(data);
			if (data.message.errno != 0) {
			...
			if ((data.message.message.pages <= (total/50)) || (!data.message.message.total && !data.message.message.offset)) {
				...
				download(data.message.message.offset);
			}
		});
	}
